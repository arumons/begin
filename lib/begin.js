(function() {
  ...
  ArrayUnits = (function() {
    ...
    ArrayUnits.prototype.each = function(block, thisp) {
      ...
      return new Units(function(array) {
        array.forEach(function(item, index, array) {
          return units.then(defed.call(thisp, item, index, array)).then(function() {
            var args;
            args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            result.push(args);
            return this.next();
  ...
  begin.filter = function(arr, block, thisp) {
    return begin(arr.filter(block, thisp));
  };
  begin.each = function(arr, block, thisp) {
    return begin(arr.each(block, thisp));
  };
  begin.every = function(arr, block, thisp) {
    return beign(arr.every(block, thisp));
  };
  begin.some = function(arr, block, thisp) {
    return begin(arr.some(block, thisp));
  };
  begin.reduce = function(arr, block, init) {
    return begin(arr.reduce(block, init));
  };
  begin.reduceRight = function(arr, block, init) {
    return begin(arr.reduceRight(block, init));
  };
